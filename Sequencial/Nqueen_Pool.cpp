...
void emitter(int depth, int midle, int is_even, int x,int init_row,int end_row,int init_column,int end_column,Chess_Board *Chess_Table){
int x;	
	if(x){
		for(i=0; i < midle i++){
...
#endif
					emitter(depth,midle,is_even,0,0,end_row,(init_column+1),end_column,Chess_Table);
		...
		for(int i=init_row; i < end_row; i++){//Will be evaluate all the rows
			if(Chess_Table->Valid_Op(i,init_column) && (init_Row < end_Row)&& (init_column < end_Col)){
				if((depth==init_column)){
					/*split the computation*/
					Chess_Board *chess_table_cp = new Chess_Board(&Chess_Table);
					variables * Start_var=new variables(chess_table_cp,0,0,end_row,(init_column+1),end_column,0,medle,0);
...
emitter(1,temp_medle,0,1,0,(queen_number-1),0,(queen_number-1),chess_table);	
