...
Board::Board(int in_size){
	size=in_size;
	 std::vector< std::vector<int> >temp_board(in_size,std::vector<int>(in_size,0));
	 std::vector<int>temp_column(in_size,0);
	 control_column=temp_column;
	 board_table=temp_board; 
}



Board::Board(int in_size, int initx, int intity){
	size=in_size;
	 std::vector< std::vector<int> >temp_board(in_size,std::vector<int>(in_size,0));
	 std::vector<int>temp_column(in_size,0);
	 temp_board[initx][intity] =1;
	 temp_column[intity]=1;
	 control_column=temp_column;
	 board_table=temp_board; 
}

char Board::Complete_Computation(void){
	char _success = count (control_column.begin(), control_column.end(), 0);
	if(_success)return 0;
	return 1;
}

char Board::isSafe(int row, int col){
	int i, j;
	/*check if this colunm is free */
	if(control_column[col]) return 0;
	/* Check this row on left side */
	for (i = 0; i < col; i++)
        if (board_table[row][i])
            return 0;
