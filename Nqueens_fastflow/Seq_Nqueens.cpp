...
void Nqueens (Chess_Board *Chess_Table, int x, int init_Row, int end_Row, int init_Col, int end_Col, int midle,int is_even, int *rest, bool print)
{
	int i;
	if(Chess_Table->complete()){
		*rest=*rest+1;
		...
		if(midle==0){range=(Chess_Table->size)/2;}
			for(i=init_Row; i < end_Row; i++ ){/*Will be evaluated only the fist three row*/
...
#endif
					Nqueens(Chess_Table,0,0,end_Row,(init_Col+1),end_Col,midle,is_even,rest,print);
